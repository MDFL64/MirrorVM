...
namespace MirrorVM
{
	...
	class IRBuilder
	{
		...
		public IRBuilder( List<ValType> local_types, List<ValType> ret_types, int arg_count )
		{
			ArgCount = arg_count;
			VariableCount = local_types.Count;
			ReturnSlotCount = ret_types.Count;
		...
		public void AddReturn( int ret_count )
		{
			...
			for ( int i = 0; i < values.Length; i++ )
			{
				...
				AddStatement( new Local( i, ty, LocalKind.Frame ), values[i] );
			}
		...
		private void LowerLocal( Expression e )
		{
			if ( e is Local local )
			{
				...
				else if ( local.Kind == LocalKind.Call )
				{
					...
					local.Index += GetCallBase();
				}
				else if ( local.Kind == LocalKind.Frame | local.Kind == LocalKind.Register )
				{
				...
				{
					throw new Exception( "FIX " + local.Kind );
				}
				...
				if ( Config.REG_ALLOC_MODE == RegAllocMode.Basic || Config.REG_ALLOC_MODE == RegAllocMode.None )
				{
					...
					{
						int reg_index = local.Index - ArgCount;
						if ( reg_index >= 0 && reg_index < reg_count )
						{
							local.Kind = LocalKind.Register;
							local.Index = reg_index;
							TOTAL_REG++;
						}
						else
						{
							int index = ReturnSlotCount + local.Index;
							local.Kind = LocalKind.Frame;
							local.Index = index;
							TOTAL_FRAME++;
							FRAME_INDICES[index]++;
