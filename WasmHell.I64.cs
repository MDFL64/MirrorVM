...
struct Op_I64_EqualZero<A> : Expr<int> where A: struct, Expr<long>
{
    ...
    public int Run(Registers reg, Span<long> frame, WasmInstance inst) => default(A).Run(reg, frame, inst) == 0 ? 1 : 0;
}
...
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public int Run(Registers reg, Span<long> frame, WasmInstance inst) => (ulong)default(A).Run(reg, frame, inst) >= (ulong)default(B).Run(reg, frame, inst) ? 1 : 0;
}
...
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public long Run(Registers reg, Span<long> frame, WasmInstance inst) => default(C).Run();
}
struct Op_I64_Add<A,B> : Expr<long> where A: struct, Expr<long> where B: struct, Expr<long> {
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public long Run(Registers reg, Span<long> frame, WasmInstance inst) => default(A).Run(reg, frame, inst) + default(B).Run(reg, frame, inst);
}
...
struct Op_I64_Div_U<A,B> : Expr<long> where A: struct, Expr<long> where B: struct, Expr<long> {
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public long Run(Registers reg, Span<long> frame, WasmInstance inst) => (long)((ulong)default(A).Run(reg, frame, inst) / (ulong)default(B).Run(reg, frame, inst));
}
struct Op_I64_Rem_S<A,B> : Expr<long> where A: struct, Expr<long> where B: struct, Expr<long> {
    ...
    public long Run(Registers reg, Span<long> frame, WasmInstance inst) {
        long a = default(A).Run(reg, frame, inst);
        long b = default(B).Run(reg, frame, inst);
...
struct Op_I64_ShiftLeft<A,B> : Expr<long> where A: struct, Expr<long> where B: struct, Expr<long> {
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public long Run(Registers reg, Span<long> frame, WasmInstance inst) => default(A).Run(reg, frame, inst) << (int)default(B).Run(reg, frame, inst);
}
...
struct Op_I64_RotateLeft<A,B> : Expr<long> where A: struct, Expr<long> where B: struct, Expr<long> {
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public long Run(Registers reg, Span<long> frame, WasmInstance inst) => (long)BitOperations.RotateLeft((ulong)default(A).Run(reg, frame, inst),(int)default(B).Run(reg, frame, inst));
}
struct Op_I64_RotateRight<A,B> : Expr<long> where A: struct, Expr<long> where B: struct, Expr<long> {
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public long Run(Registers reg, Span<long> frame, WasmInstance inst) => (long)BitOperations.RotateRight((ulong)default(A).Run(reg, frame, inst),(int)default(B).Run(reg, frame, inst));
}
struct Op_I64_LeadingZeros<A> : Expr<long> where A: struct, Expr<long>
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public long Run(Registers reg, Span<long> frame, WasmInstance inst) => BitOperations.LeadingZeroCount((ulong)default(A).Run(reg, frame, inst));
}
struct Op_I64_TrailingZeros<A> : Expr<long> where A: struct, Expr<long>
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public long Run(Registers reg, Span<long> frame, WasmInstance inst) => BitOperations.TrailingZeroCount((ulong)default(A).Run(reg, frame, inst));
}
struct Op_I64_PopCount<A> : Expr<long> where A: struct, Expr<long>
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public long Run(Registers reg, Span<long> frame, WasmInstance inst) => BitOperations.PopCount((ulong)default(A).Run(reg, frame, inst));
}
struct Op_I64_Extend8_S<A> : Expr<long> where A: struct, Expr<long>
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public long Run(Registers reg, Span<long> frame, WasmInstance inst) => (sbyte)default(A).Run(reg, frame, inst);
}
struct Op_I64_Extend16_S<A> : Expr<long> where A: struct, Expr<long>
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public long Run(Registers reg, Span<long> frame, WasmInstance inst) => (short)default(A).Run(reg, frame, inst);
}
struct Op_I64_Extend32_S<A> : Expr<long> where A: struct, Expr<long>
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public long Run(Registers reg, Span<long> frame, WasmInstance inst) => (int)default(A).Run(reg, frame, inst);
}
...
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public long Run(Registers reg, Span<long> frame, WasmInstance inst) => default(A).Run(reg, frame, inst);
}
struct Op_I64_Extend_I32_U<A> : Expr<long> where A: struct, Expr<int>
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public long Run(Registers reg, Span<long> frame, WasmInstance inst) => (uint)default(A).Run(reg, frame, inst);
}
struct Op_I64_Truncate_F32_S<A> : Expr<long> where A: struct, Expr<float>
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public long Run(Registers reg, Span<long> frame, WasmInstance inst) => checked((long)default(A).Run(reg, frame, inst));
}
struct Op_I64_Truncate_F32_U<A> : Expr<long> where A: struct, Expr<float>
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public long Run(Registers reg, Span<long> frame, WasmInstance inst) => (long)checked((ulong)default(A).Run(reg, frame, inst));
}
...
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public long Run(Registers reg, Span<long> frame, WasmInstance inst) => (long)default(A).Run(reg, frame, inst);
}
struct Op_I64_TruncateSat_F32_U<A> : Expr<long> where A: struct, Expr<float>
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public long Run(Registers reg, Span<long> frame, WasmInstance inst) => (long)(ulong)default(A).Run(reg, frame, inst);
}
...
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public long Run(Registers reg, Span<long> frame, WasmInstance inst) => BitConverter.DoubleToInt64Bits(default(A).Run(reg, frame, inst));
}
