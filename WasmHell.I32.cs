...
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public int Run(Registers reg, Span<long> frame, WasmInstance inst) => (uint)default(A).Run(reg, frame, inst) >= (uint)default(B).Run(reg, frame, inst) ? 1 : 0;
}
...
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public int Run(Registers reg, Span<long> frame, WasmInstance inst) => (int)default(C).Run();
}
struct Op_I32_Add<A,B> : Expr<int> where A: struct, Expr<int> where B: struct, Expr<int> {
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public int Run(Registers reg, Span<long> frame, WasmInstance inst) => default(A).Run(reg, frame, inst) + default(B).Run(reg, frame, inst);
}
...
struct Op_I32_Div_U<A,B> : Expr<int> where A: struct, Expr<int> where B: struct, Expr<int> {
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public int Run(Registers reg, Span<long> frame, WasmInstance inst) => (int)((uint)default(A).Run(reg, frame, inst) / (uint)default(B).Run(reg, frame, inst));
}
struct Op_I32_Rem_S<A,B> : Expr<int> where A: struct, Expr<int> where B: struct, Expr<int> {
    ...
    public int Run(Registers reg, Span<long> frame, WasmInstance inst) {
        int a = default(A).Run(reg, frame, inst);
        int b = default(B).Run(reg, frame, inst);
...
struct Op_I32_RotateLeft<A,B> : Expr<int> where A: struct, Expr<int> where B: struct, Expr<int> {
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public int Run(Registers reg, Span<long> frame, WasmInstance inst) => (int)BitOperations.RotateLeft((uint)default(A).Run(reg, frame, inst),default(B).Run(reg, frame, inst));
}
struct Op_I32_RotateRight<A,B> : Expr<int> where A: struct, Expr<int> where B: struct, Expr<int> {
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public int Run(Registers reg, Span<long> frame, WasmInstance inst) => (int)BitOperations.RotateRight((uint)default(A).Run(reg, frame, inst),default(B).Run(reg, frame, inst));
}
struct Op_I32_LeadingZeros<A> : Expr<int> where A: struct, Expr<int>
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public int Run(Registers reg, Span<long> frame, WasmInstance inst) => BitOperations.LeadingZeroCount((uint)default(A).Run(reg, frame, inst));
}
struct Op_I32_TrailingZeros<A> : Expr<int> where A: struct, Expr<int>
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public int Run(Registers reg, Span<long> frame, WasmInstance inst) => BitOperations.TrailingZeroCount((uint)default(A).Run(reg, frame, inst));
}
struct Op_I32_PopCount<A> : Expr<int> where A: struct, Expr<int>
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public int Run(Registers reg, Span<long> frame, WasmInstance inst) => BitOperations.PopCount((uint)default(A).Run(reg, frame, inst));
}
struct Op_I32_Extend8_S<A> : Expr<int> where A: struct, Expr<int>
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public int Run(Registers reg, Span<long> frame, WasmInstance inst) => (sbyte)default(A).Run(reg, frame, inst);
}
struct Op_I32_Extend16_S<A> : Expr<int> where A: struct, Expr<int>
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public int Run(Registers reg, Span<long> frame, WasmInstance inst) => (short)default(A).Run(reg, frame, inst);
}
...
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public int Run(Registers reg, Span<long> frame, WasmInstance inst) => (int)default(A).Run(reg, frame, inst);
}
struct Op_I32_Truncate_F32_S<A> : Expr<int> where A: struct, Expr<float>
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public int Run(Registers reg, Span<long> frame, WasmInstance inst) => checked((int)default(A).Run(reg, frame, inst));
}
struct Op_I32_Truncate_F32_U<A> : Expr<int> where A: struct, Expr<float>
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public int Run(Registers reg, Span<long> frame, WasmInstance inst) => (int)checked((uint)default(A).Run(reg, frame, inst));
}
...
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public int Run(Registers reg, Span<long> frame, WasmInstance inst) => (int)(uint)default(A).Run(reg, frame, inst);
}
...
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public int Run(Registers reg, Span<long> frame, WasmInstance inst) => BitConverter.SingleToInt32Bits(default(A).Run(reg, frame, inst));
}
