...
namespace MirrorVM
{
    ...
    struct GetGlobal_I32<INDEX> : Expr<int> where INDEX : struct, Const
    {
        ...
        public int Run( ref Registers reg, Span<long> frame, WasmInstance inst ) => (int) inst.Globals[(int)default( INDEX ).Run()];
    }

    struct GetGlobal_I64<INDEX> : Expr<long> where INDEX : struct, Const
    {
        [MethodImpl( MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization )]
        public long Run( ref Registers reg, Span<long> frame, WasmInstance inst ) =>  inst.Globals[(int)default( INDEX ).Run()];
    }

    struct GetGlobal_F32<INDEX> : Expr<float> where INDEX : struct, Const
    {
        [MethodImpl( MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization )]
        public float Run( ref Registers reg, Span<long> frame, WasmInstance inst ) => BitConverter.Int32BitsToSingle( (int) inst.Globals[(int)default( INDEX ).Run()] );
    }

    struct GetGlobal_F64<INDEX> : Expr<double> where INDEX : struct, Const
    {
        [MethodImpl( MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization )]
        public double Run( ref Registers reg, Span<long> frame, WasmInstance inst ) => BitConverter.Int64BitsToDouble(  inst.Globals[(int)default( INDEX ).Run()] );
    }
