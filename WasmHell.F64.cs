...
{
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public double Run(Registers reg, Span<long> frame, WasmInstance inst) => BitConverter.Int64BitsToDouble(default(C).Run());
}
struct Op_F64_Add<A,B> : Expr<double> where A: struct, Expr<double> where B: struct, Expr<double> {
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public double Run(Registers reg, Span<long> frame, WasmInstance inst) => FloatHelper.CanonicalF64( default(A).Run(reg, frame, inst) + default(B).Run(reg, frame, inst) );
}
...
struct Op_F64_Min<A,B> : Expr<double> where A: struct, Expr<double> where B: struct, Expr<double> {
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public double Run(Registers reg, Span<long> frame, WasmInstance inst) => FloatHelper.CanonicalF64( Math.Min(default(A).Run(reg, frame, inst), default(B).Run(reg, frame, inst)) );
}
struct Op_F64_Max<A,B> : Expr<double> where A: struct, Expr<double> where B: struct, Expr<double> {
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public double Run(Registers reg, Span<long> frame, WasmInstance inst) => FloatHelper.CanonicalF64( Math.Max(default(A).Run(reg, frame, inst), default(B).Run(reg, frame, inst)) );
}
struct Op_F64_CopySign<A,B> : Expr<double> where A: struct, Expr<double> where B: struct, Expr<double> {
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public double Run(Registers reg, Span<long> frame, WasmInstance inst) => Math.CopySign(default(A).Run(reg, frame, inst), default(B).Run(reg, frame, inst));
}
...
struct Op_F64_Neg<A> : Expr<double> where A: struct, Expr<double> {
    // canonicalization not required by spec
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public double Run(Registers reg, Span<long> frame, WasmInstance inst) => -default(A).Run(reg, frame, inst);
}
struct Op_F64_Abs<A> : Expr<double> where A: struct, Expr<double> {
    // canonicalization not required by spec
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public double Run(Registers reg, Span<long> frame, WasmInstance inst) => Math.Abs(default(A).Run(reg, frame, inst));
}
struct Op_F64_Sqrt<A> : Expr<double> where A: struct, Expr<double> {
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public double Run(Registers reg, Span<long> frame, WasmInstance inst) => FloatHelper.CanonicalF64( Math.Sqrt(default(A).Run(reg, frame, inst)) );
}
struct Op_F64_Floor<A> : Expr<double> where A: struct, Expr<double> {
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public double Run(Registers reg, Span<long> frame, WasmInstance inst) => FloatHelper.CanonicalF64( Math.Floor(default(A).Run(reg, frame, inst)) );
}
struct Op_F64_Ceil<A> : Expr<double> where A: struct, Expr<double> {
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public double Run(Registers reg, Span<long> frame, WasmInstance inst) => FloatHelper.CanonicalF64( Math.Ceiling(default(A).Run(reg, frame, inst)) );
}
struct Op_F64_Truncate<A> : Expr<double> where A: struct, Expr<double> {
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public double Run(Registers reg, Span<long> frame, WasmInstance inst) => FloatHelper.CanonicalF64( Math.Truncate(default(A).Run(reg, frame, inst)) );
}
struct Op_F64_Nearest<A> : Expr<double> where A: struct, Expr<double> {
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public double Run(Registers reg, Span<long> frame, WasmInstance inst) => FloatHelper.CanonicalF64( Math.Round(default(A).Run(reg, frame, inst)) );
}
...
struct Op_F64_Convert_I32_U<A> : Expr<double> where A: struct, Expr<int> {
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public double Run(Registers reg, Span<long> frame, WasmInstance inst) => (uint)default(A).Run(reg, frame, inst);
}
...
struct Op_F64_Reinterpret_I64<A> : Expr<double> where A: struct, Expr<long> {
    [MethodImpl(MethodImplOptions.AggressiveInlining | MethodImplOptions.AggressiveOptimization)]
    public double Run(Registers reg, Span<long> frame, WasmInstance inst) => BitConverter.Int64BitsToDouble(default(A).Run(reg, frame, inst));
}
